...
class LitSegNet(pl.LightningModule):
        ...
        if not model_only:
            ...
            if not (self.hparams.dataset == "combo"):
                ...
                if self.hparams.dataset_combo is None:
                    self.hparams.dataset_combo = "cityscapes,mapillary,idd,tas500,ycor,rugd,bdd,acdc"
                    #self.hparams.dataset_combo = "cityscapes,tas500"
    ...
    def predict(self, batch, set, dataset=None, save=False, batch_idx=None):

        #third_tensor = torch.cat((first_tensor, second_tensor), 0)
        # print(dataset)
        if dataset == "combo":
            x = torch.cat([batch[i]["sample"][0] for i in range(len(batch))], 0)
            # print(batch[0]["filename"])
            y = torch.cat([batch[i]["sample"][1] for i in range(len(batch))], 0)
    ...
    def train_dataloader(self):
        if not (self.hparams.dataset == "combo"):
            ...
            loaders = [
                DataLoader(set, batch_size=1, num_workers=self.hparams.workers, shuffle=True) for name,set in self.train_set.items()
                ]
            print(loaders)
            return loaders
    ...
    def val_dataloader(self):
        if not (self.hparams.dataset == "combo"):
            ...
            loaders = [
                DataLoader(set, batch_size=self.hparams.bs, num_workers=self.hparams.workers, shuffle=False) for name,set in self.val_set.items()
                ]
            print(loaders)
            return loaders
    ...
    def test_dataloader(self):
        if not (self.hparams.dataset == "combo") or self.test_samples == "test":
            ...
            loaders = {
                name:DataLoader(set, batch_size=self.hparams.bs, num_workers=self.hparams.workers, shuffle=False) for name,set in self.test_set.items()
                }
        return loaders
