...
do
  ...
  checkpoint1="2021-06-11 13-18-cityscapes-c3-sord-1,2,3-a1-logl2-lw-rgb-epoch=32-val_loss=0.0044"
...
do
  # checkpoint1="2021-05-12 11-24-combo-c30-sord-1,2,3-a1-logl2-rgb-epoch=42-val_loss=0.0060"
  checkpoint1="2021-06-11 22-51-freiburgthermal-c13-sord-1,2,3-a1-logl2-ir-epoch=63-val_loss=0.0059"
  txtoutput="results/${dataset}/${xp}/txt/${checkpoint1}.txt"
  ...
  if [ ! -f "$txtoutput" ] || [ $isInFile -eq 0 ] ; then
    ...
    echo "Running evaluation for ${dataset} | ${checkpoint1}"
    if [ -f "lightning_logs/${checkpoint1}.ckpt" ] ; then
      python3 lightning.py --dataset $dataset --bs 1 --save --save_xp $xp --save --test_checkpoint "lightning_logs/${checkpoint1}.ckpt" --modalities ir > "$txtoutput" 2>&1
    else
      echo "checkpoint ${checkpoint1} not found :( bye"
    fi
  fi
  ...
  checkpoint2="2021-05-06 13-48-freiburgthermal-c13-sord-1,2,3-a1-logl2-rgb-epoch=23-val_loss=0.0037"
  txtoutput="results/${dataset}/${xp}/txt/${checkpoint2}.txt"
