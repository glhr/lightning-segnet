...
for d in ${!datasets[@]}
  ...
  do
        ...
        isInFile=$(cat "$txtoutput" | grep -c "DATALOADER:0 TEST RESULTS")
          ...
          python3 lightning.py --bs 1 --dataset $dataset --test_checkpoint "lightning_logs/${checkpoints[$i]}" ${commands[$i]} --gpus 1 --test_set ${datasetssets[$dataset]} --save --save_xp $xp #> "$txtoutput" 2>&1
