...
def _confusion_matrix_update(
    ...
    unique_mapping = (target[mask].view(-1) * num_classes + preds[mask].view(-1)).to(torch.long)
...
def _confusion_matrix_compute(confmat: torch.Tensor, normalize: Optional[str] = None, ignore_index=None) -> torch.Tensor:
